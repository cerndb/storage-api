...
class VolumeREST(Resource):
    ...
    def post(self, volname):
        ...
        if vendor == 'NetApp':
            try:
                netapp = NetAppprov(clustername, volname, initsize, maximumsize, incrementsize, vserver, policy,
                                    junctionpath, typeaggr, ip, snapenable, business)
                result = netapp.CreateVolume()
            except Exception as ex:
                exc_type, exc_value, exc_traceback = sys.exc_info()
                VolumeREST.logger.debug('problem creating volume : ' + str(ex))
                VolumeREST.logger.debug(
                    'problem creating volume : ' + repr(traceback.format_exception(exc_type, exc_value, exc_traceback)))
