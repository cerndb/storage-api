...
class BasicStorage(object):
    ...
    def __init__(self, serverpath):
        ...
        BasicStorage.logger.debug("Begin")
        assert serverpath.count(':') == 1, \
            "too many <:> in the tuple controller serverpath, this is an error"

        self.controller, self.serverlayout = serverpath.split(':', 2)
        BasicStorage.logger.debug("logger has been initialised: controller: "
                                  "<%s> controllers path: <%s>",
                                  self.controller, self.serverlayout)
        helper = re.compile(r'^(\D{6,})\d{1,}.*$')
        # Exception: AttributeError: 'NoneType' object has no attribute 'groups
        self.clustertoconnect = helper.search(self.controller).groups()
        assert len(self.clustertoconnect) >= 1, \
            "we couldnt extract which cluster are you work" \
            "ing with: %s" % self.controller

        BasicStorage.logger.debug("serverlayout is: " + self.serverlayout)
        assert re.search('^[/\-\w]+$', self.serverlayout), \
            "remote path contains strange characters"
