...
class NetAppops(BasicStorage):
    ...
    def CreateServer(self, clustername, vserver=0):
        ...
        if vserver != 0:
            NetAppops.logger.debug("working with IP: %s and user: %s", CONFIG[clustername][vserver]['mgmt-ip'],
                                   CONFIG[clustername][vserver]['admin'])
            ...
            NetAppops.logger.debug("working with IP: %s and user: %s", CONFIG[clustername]['mgmt-ip'],
                                   CONFIG[clustername]['admin'])
...
#       print("hostname  verification: ",s.is_hostname_verification_enabled())
    ...
    def GetInfoPath(self, admin=1):
        ...
        while True:
            ...
            if isinstance(resp, NaElement) and resp.results_errno() != 0:
                NetAppops.logger.error("query failed: reason %s error number: %s ", resp.results_reason(),
                                       resp.results_errno())
            ...
            if isinstance(resp, NaElement) and resp.child_get_int("num-records") > 1:
                NetAppops.logger.error("query returned %s records, when just one is expected.",
                                       resp.child_get_int("num-records"))
                raise StorageException("query returned %s records, when just one is expected.",
                                       resp.child_get_int("num-records"))
    ...
    def GetInfoVolandAggr(self):
        ...
        aggrVol = {"aggr_name": self.volume["aggregate"], "aggr_size_used": aggrusedsize,
                   "aggr_size_free": aggrfreesize, "aggr_owner": nodename}
    ...
    def GetSnapshotsList(self):
        ...
        if ((snapshots is None) or (snapshots == "")):
        ...
        for ss in snapshotsList:
            ...
            snaps.append(
                '{:>23} {:>24} {} {:>10} {:>10} {:>10}'.format(name, time.strftime("%Y%m%d_%H%M%S", date), busy, total,
                                                               cumtotal, dependency))
    ...
    def CreateSnapshot(self, name=0):
        ...
        if (resp.results_errno() != 0):
            NetAppops.logger.error("while creating an snapshot: reason %s error number: %s ", resp.results_reason(),
                                   resp.results_errno())
            raise StorageException("while creating an snapshot: reason %s error number: %s ", resp.results_reason(),
                                   resp.results_errno())
    ...
    def RestoreSnapshot(self, name):
        ...
        if (resp.results_errno() != 0):
            NetAppops.logger.error("while restoring snapshot %s on volume %s: reason %s error number: %s ", name,
                                   self.volume["name"], resp.results_reason(), resp.results_errno())
            raise StorageException("while restoring  snapshot %s on volume %s: reason %s error number: %s ", name,
                                   self.volume["name"], resp.results_reason(), resp.results_errno())
    ...
    def CloneSnapshot(self, snapshot=0, junction_path=0):
        ...
        if (resp.results_errno() != 0):
            NetAppops.logger.error("while cloning snapshot %s on volume %s: reason %s error number: %s ", snapshot,
                                   self.volume["name"], resp.results_reason(), resp.results_errno())
            raise StorageException("while cloning snapshot %s on volume %s: reason %s error number: %s ", snapshot,
                                   self.volume["name"], resp.results_reason(), resp.results_errno())
    ...
    def DeleteSnapshot(self, name):
        ...
        if (resp.results_errno() != 0):
            NetAppops.logger.error("while deleting snapshot %s on volume %s: reason %s error number: %s ", name,
                                   self.volume["name"], resp.results_reason(), resp.results_errno())
            raise StorageException("while deleting  snapshot %s on volume %s: reason %s error number: %s ", name,
                                   self.volume["name"], resp.results_reason(), resp.results_errno())
