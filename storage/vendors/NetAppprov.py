...
class NetAppprov(NetAppops,BasicStorage):
	def __init__(self,clustername,volname,initial_size,final_size,increment,vserver,firewall_name,junction_path, type,firewall_ip=0,snaps=0, business=0):
				...
				if CONFIG[self.business][self.vserver]["nodes"][key][self.type]:
					...
					for key2 in CONFIG[self.business][self.vserver]["nodes"][key][self.type]:
						...
						if comodin in dict_aggrpernode:
							...
							NetAppprov.logger.debug("%s size %s beeing added to candidate",comodin,str(dict_aggrpernode[comodin]))							
		else:
			NetAppprov.logger.debug("End")
			return None
		#sorted
	...
	def CreateVolume(self):
		...
		if volume is not None:
			NetAppprov.logger.debug("volume with %s junction-path exists with name: %s",self.junction_path,volume['name'])
			NetAppprov.logger.debug("end")
			return 0
		else:	
	...
	def SetAutoSize(self,maxsize,increment):
		...
		NetAppprov.logger.debug("autosize for volume: %s set to a maximum size of %s GB and increment of %s GB.",self.volname,maxsize,increment)
		NetAppprov.logger.debug("end")
		return 0
